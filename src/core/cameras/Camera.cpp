...
void Camera::requestOutputBuffers(const std::vector<OutputBufferSettings> &settings) {
    for (const auto &b : settings) {
        switch (b.type()) {
            case OutputColor:
                _colorBuffer = std::make_unique<OutputBufferVec3f>(_res, b);
                break;
            case OutputDepth:
                _depthBuffer = std::make_unique<OutputBufferF>(_res, b);
                break;
            case OutputNormal:
                _normalBuffer = std::make_unique<OutputBufferVec3f>(_res, b);
                break;
            case OutputAlbedo:
                _albedoBuffer = std::make_unique<OutputBufferVec3f>(_res, b);
                break;
            case OutputVisibility:
                _visibilityBuffer = std::make_unique<OutputBufferF>(_res, b);
                break;
            case OutputDiffuse:
                _diffuseBuffer = std::make_unique<OutputBufferVec3f>(_res, b);
                break;
            case OutputSpecular:
                _specularBuffer = std::make_unique<OutputBufferVec3f>(_res, b);
            default:
                break;
        }
    }
...
void Camera::requestColorBuffer() {
    if (!_colorBuffer) {
        _colorBuffer = std::make_unique<OutputBufferVec3f>(_res, _colorBufferSettings);
    }
    _colorBufferWeight = 1.0;
}

void Camera::requestSplatBuffer() {
    _splatBuffer = std::make_unique<AtomicFramebuffer>(_res.x(), _res.y(), _filter);
    _splatWeight = 1.0;
}
