...
public:
    ...
    auto *diffuseBuffer() { return _diffuseBuffer.get(); }
    auto *specularBuffer() { return _specularBuffer.get(); }
    ...
    [[nodiscard]] const OutputBufferVec3f *colorBuffer() const {
        return _colorBuffer.get();
    }
    
    [[nodiscard]] const auto *diffuseBuffer() const noexcept { return _diffuseBuffer.get(); }
    [[nodiscard]] const auto *specularBuffer() const noexcept { return _specularBuffer.get(); }
    
    [[nodiscard]] const OutputBufferF *depthBuffer() const {
        return _depthBuffer.get();
    }
    
    [[nodiscard]] const OutputBufferVec3f *normalBuffer() const {
        return _normalBuffer.get();
    }
    
    [[nodiscard]] const OutputBufferVec3f *albedoBuffer() const {
        return _albedoBuffer.get();
    }
    
    [[nodiscard]] const OutputBufferF *visibilityBuffer() const {
        return _visibilityBuffer.get();
    }
    
    [[nodiscard]] inline Vec3f tonemap(const Vec3f &c) const {
        return Tonemap::tonemap(_tonemapOp, max(c, Vec3f(0.0f)));
    }
    
    [[nodiscard]] inline Vec3f getLinear(int x, int y) const {
        int idx = x + y * _res.x();
        Vec3f result(0.0f);
        if (_colorBuffer) {
    ...
    [[nodiscard]] inline Vec3f get(int x, int y) const {
